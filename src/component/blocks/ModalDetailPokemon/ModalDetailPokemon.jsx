...
import { Container, List, ListAbilities, Bar, CustomTitle, Left, Right, CustomSubTitle, Img, Relative } from './styles'

const ModalDetailPokemon = () => {
  const detailPokemon = useSelector(state => state.pokemons.currentPokemon)
          ...
          <Relative>
            ...
            <ListAbilities color={getPrimaryColor(detailPokemon.types)}>
              {detailPokemon.abilities.map(item => (
                <li key={item}>{formatTextToCapitalizeWithTrace(item.ability.name)}</li>
              ))}
            </ListAbilities>
          </Relative>
        </Left>
